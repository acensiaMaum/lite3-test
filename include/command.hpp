...
// Serialize only the header to bytes (network byte order)
// Returns true on success; false if paramters_size is invalid.
...
// Writes header and only head.paramters_size bytes of data (not the entire array).
// Returns true on success; false if paramters_size is invalid.
...
// Utility to stringify only the CommandHead
